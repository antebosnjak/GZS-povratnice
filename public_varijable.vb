Module public_varijable
    Public putanja_povratnice As String




    Public ime_prezime_operator As String
    Public zupanija_skeniranja As String
    Public lokacija_izmjere As String
    Public nadlezni_odjel As String


    Public novi_radni_dan As String
    'Public trenutni_radni_dan As String
    Public naziv_grupe As String
    Public broj_skena As Integer
    Public putanja_slika As String
    Public putanja_txt As String
    Public putanja_pdf As String
    Public putanja_tiff As String
    Public putanja_jpgtemp As String



    Public putanja_db As String

    Public putanja_grupe As String

    Public ime_grupe_folder As String




    Public dbname As String
    Public dbpath As String

    Public imetiff As String
    Public imepdf As String



    'Varijable za export

    Public pdf_potpisani As String
    Public tiff_spojeni As String



    Public imesqlitedatoteke As String



    Public ftp_slanje As Boolean

    Public start_radnog_dana As Boolean

    Public vrsta_akta As String

    Public jedanaest As Boolean

    Public vrsta_prijenosa As String
    Public putanja_servera As String
    Public putanja_prijenosa As String




    Public broj_povratnice As String

End Module
